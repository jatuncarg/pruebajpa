...
public class FrmManteProd extends JFrame {
	...
	void llenaCombo() {
		...
		List<Categoria> lstCategorias = em.createQuery("select c from Categoria c").
				getResultList();
		cboCategorias.addItem("Seleccione"); // opción 0
	...
	void listado() {
		...
		List<Producto> lstProductos = em.createQuery("select p from Producto p").
				getResultList();
		txtSalida.setText("");
		for (Producto p : lstProductos) {
			txtSalida.append(p.getId_prod() + " " + 
			p.getDes_prod() + " " + p.getStk_prod() + " " + 
			p.getPre_prod() + " " + p.getIdcategoria() + "\n");
		}
